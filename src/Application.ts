...
class Application<T, K extends keyof T> implements IApplication<T, K> {
  ...
  treeShake({ storeKey, changedValue }: { storeKey: K; changedValue: object }) {
    ...
    const compare = (
      ...
      const keysToCompare2 = Object.keys(branch.children);

      keysToCompare2.forEach(key => {
        ...
        if (isTypeEqual(oldValue, newValue)) {
          ...
          if (isNumber(newValue) || isString(newValue)) {
            if (oldValue !== newValue) {
              if (branch.children[key].patchers.length) {
                branch.children[key].patchers.forEach(patcher => {
                  patchers.push({ collections, patcher, operation });
                });
