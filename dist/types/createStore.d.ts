...
export declare type GetReducerPayload<T> = T extends (S: any) => object ? never : T extends (S: any, payload: infer B) => object ? B extends any ? B : never : never;
...
export declare type SafeDispatch<T, KM extends {} = {}, OKM extends keyof KM = keyof KM, TK extends GetTotalKey<T> = GetTotalKey<T>, MK extends OKM | TK = OKM | TK, P extends KeyValueTupleToObject<GetMergedPayload<T>> = KeyValueTupleToObject<GetMergedPayload<T>>> = <A extends MK = MK>(action: SafeAction<MK, P, A> | Array<SafeAction<MK, P, void>>) => void;
export declare type SafeAction<T, P, A> = {
    type: A extends string ? A : T;
    payload?: A extends string ? A extends keyof P ? P[A] : never : T extends keyof P ? P[T] : never;
};
export {};
